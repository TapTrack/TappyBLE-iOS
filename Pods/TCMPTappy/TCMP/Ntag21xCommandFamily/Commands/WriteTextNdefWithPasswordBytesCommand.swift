//
//  WriteTextNdefWithPasswordBytesCommand.swift
//  TappyBLE
//
...
@objc public class WriteTextNdefWithPasswordBytesCommand: NSObject, TCMPMessage, WriteNtag21xWithPasswordBytesCommand {
    
    @objc public let commandFamily: [UInt8] = CommandFamily.ntag21x
    
    @objc public let commandCode: UInt8 = Ntag21xCommandCode.writeTextNdefWithPasswordBytes.rawValue
    ...
    @objc public convenience init?(timeout: UInt8, readProtection: Bool, password: [UInt8],
                                passwordAcknowledgement: [UInt8], text: String) {
        self.init(timeout: timeout, readProtection: readProtection, password: password,
                  passwordAcknowledgement: passwordAcknowledgement, content: Array(text.utf8))
    }
