//
//  WriteCustomNDEFWithPasswordCommand.swift
//  TappyBLE
//
...
@objc public class WriteCustomNDEFWithPasswordCommand: NSObject, TCMPMessage, WriteNtag21xWithPasswordCommand {
    
    @objc public let commandFamily: [UInt8] = CommandFamily.ntag21x
    
    @objc public let commandCode: UInt8 = Ntag21xCommandCode.writeCustomNdefWithPassword.rawValue
    ...
    @objc public var uri: String {
        get {
            let uriString = String(bytes: content, encoding: .utf8)
            return (uriString != nil) ? uriString! : ""
        }
    }
    ...
    @objc public convenience init?(timeout: UInt8, readProtection: Bool, passwordString: String, content: [UInt8]) {
        self.init(timeout: timeout, readProtection: readProtection, password: Array(passwordString.utf8), content: content)
    }
